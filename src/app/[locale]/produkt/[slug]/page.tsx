...
import { JsonLd, productSchema, breadcrumbSchema, generateHreflangAlternates } from "@/lib/seo";
...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  const product = await getLocalizedProductBySlug(locale as Locale, slug);
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  return (
    <>
      <JsonLd data={productSchema(product, locale as Locale)} />
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div>
          ...
          {product.sku && (
            <p className="mt-1.5 text-xs text-gray-400">{t("sku")}: {product.sku}</p>
          )}
          ...
          <p className="mt-1 text-xs text-gray-500">{t("inclVat")}</p>
